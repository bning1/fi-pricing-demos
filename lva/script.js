...
function computeLVA() {
  ...
  const horizon_years = parseFloat(document.getElementById('horizon').value) || 0.25;
  ...
  if (tenor <= 0 || horizon_years <= 0) {
    document.getElementById('results').innerHTML = '<p style="color:red;">Invalid tenor or horizon.</p>';
    return;
  }
  const effective_horizon = Math.min(tenor, horizon_years);
  const steps_year = 252;
  const dt = 1 / steps_year;
  const num_steps = Math.floor(effective_horizon * steps_year);
  const times = Array.from({ length: num_steps + 1 }, (_, i) => i * dt );
  console.log(`Simming ${nsims} paths, ${num_steps} steps over ${effective_horizon.toFixed(2)}Y (H=${horizon_years.toFixed(2)})...`);
  let all_ee_paths = [];
  ...
  console.log('LVA calc', all_ee_paths.length);
  ...
  // Table
  let tableHTML = '<table style="width:100%;border-collapse:collapse;margin:1rem 0;font-size:0.95rem;"><thead><tr><th style="background:#48bb78;color:white;padding:0.75rem;border-radius:6px 0 0 0;">Horizon (Y)</th><th style="background:#48bb78;color:white;padding:0.75rem;border-radius:0 6px 0 0;">LVA | Liquidity Valuation Adjustment (liq stress H) ($M)</th></tr></thead><tbody>';
  const key_tenors = [0.25, 0.5, 1, 2, 3, 4, effective_horizon];
  key_tenors.forEach(ky => {
    const step = Math.round(ky / dt);
    const s = Math.max(0, Math.min(num_steps, step));
    const lva_val = cum_lva_m[s].toFixed(1);
    tableHTML += `<tr><td style="padding:0.5rem;border-bottom:1px solid #e2e8f0;">${ky.toFixed(2)}</td><td style="padding:0.5rem;border-bottom:1px solid #e2e8f0;font-weight:600;">${lva_val}</td></tr>`;
  });
  ...
  results.innerHTML = `
    ...
    <div>
      <p><strong>Total LVA:</strong> ${total_lva_m}M</p>
      <p><strong>Max Avg EE:</strong> ${max_ee_m}M</p>
      <p><strong>Final Avg EE (at H):</strong> ${final_avg_ee_m}M</p>
    </div>
    ...
    <details>
      ...
      <strong>LVA(H)</strong> = ∫<sub>0</sub><sup>H</sup> EE(t) × <span style="color:orange">liq premium</span> × <span style="color:green">DF(t)</span> dt (trapezoidal).<br>
      DF(t)=exp(-r t). Daily steps (252/yr), cutoff at liq horizon H.</p>
      ...
      • <strong>LVA:</strong> cost=liq premium (stress add-on), surv=1 (liq stress)</p>
    </details>
  `;
  // Charts
  const pfeCtx = document.getElementById('pfeChart')?.getContext('2d');
  if (lvaChartInst) lvaChartInst.destroy();
  lvaChartInst = new Chart(pfeCtx, {
    type: 'line',
    data: {
  ...
  cumChartInst = new Chart(histCtx, {
    ...
    options: {
      ...
      scales: {
        x: { title: { display: true, text: 'Time (Years)' } },
        y: { title: { display: true, text: 'Cum LVA ($ Millions)' }, min: 0 }
      },
      plugins: { legend: { display: true } }
    }
  });
  console.log('LVA Demo: Total', total_lva_m, 'Max EE', max_ee_m);
}
