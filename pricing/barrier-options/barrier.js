...
(function () {
  ...
  /**
   * Validate barrier option inputs by reading actual field values.
   * Returns { valid: true, params: {...} } or { valid: false, errors: [...] }.
   */
  function validateInputs(S, H, K, r, v, T) {
    var errors = [];

    if (isNaN(S) || S <= 0) errors.push('Spot price S0 must be > 0 (got: ' + S + ')');
    if (isNaN(H) || H <= 0) errors.push('Barrier H must be > 0 (got: ' + H + ')');
    if (isNaN(K) || K <= 0) errors.push('Strike K must be > 0 (got: ' + K + ')');
    if (isNaN(r))           errors.push('Rate r must be a number (got: ' + r + ')');
    if (isNaN(v) || v <= 0) errors.push('Volatility must be > 0 (got: ' + v + ')');
    if (isNaN(T) || T <= 0) errors.push('Maturity T must be > 0 (got: ' + T + ')');

    if (errors.length > 0) {
      return { valid: false, errors: errors };
    }
    return { valid: true, params: { S: S, H: H, K: K, r: r, v: v, T: T } };
  }

  // --- Reiner-Rubinstein building blocks (A–D terms) ---

  function mu_val(r, q, v) { return (r - q - 0.5 * v * v) / (v * v); }

  function lambda_val(r, q, v) {
    var m = mu_val(r, q, v);
    return Math.sqrt(m * m + 2.0 * r / (v * v));
  }

  function x1(S, K, v, T, r, q) {
    return Math.log(S / K) / (v * Math.sqrt(T)) + (1 + mu_val(r, q, v)) * v * Math.sqrt(T);
  }

  function x2(S, H, v, T, r, q) {
    return Math.log(S / H) / (v * Math.sqrt(T)) + (1 + mu_val(r, q, v)) * v * Math.sqrt(T);
  }

  function y1(H, S, K, v, T, r, q) {
    return Math.log(H * H / (S * K)) / (v * Math.sqrt(T)) + (1 + mu_val(r, q, v)) * v * Math.sqrt(T);
  }

  function y2(H, S, v, T, r, q) {
    return Math.log(H / S) / (v * Math.sqrt(T)) + (1 + mu_val(r, q, v)) * v * Math.sqrt(T);
  }
  ...
  // Term B: uses x2 (log(S/H)) but strike K in the payoff
  function B(phi, S, K, H, r, q, v, T) {
  ...
  function DIC(S, H, K, r, v, T, q) {
    ...
    if (K >= H) {
      ...
      return A(1, S, K, r, q, v, T) - B(1, S, K, H, r, q, v, T) + D(1, 1, S, K, H, r, q, v, T);
    }
  }
  ...
  function DIP(S, H, K, r, v, T, q) {
    ...
    if (S <= H) return A(-1, S, K, r, q, v, T); // already knocked in
    if (K >= H) {
      return B(-1, S, K, H, r, q, v, T) - C(-1, 1, S, K, H, r, q, v, T) + D(-1, 1, S, K, H, r, q, v, T);
    } else {
  ...
  window.BarrierPricers = {
    /** Validate inputs from actual field values. */
